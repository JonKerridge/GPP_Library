...
import GPP_Library.cluster.connectors.NodeRequestingParCastList
import GPP_Library.cluster.connectors.NodeRequestingSeqCastList
...
class Test40 {
  ...
  void test() {
    ...
    def rfl0 = new NodeRequestingSeqCastList(request: request[0].out(),
        response: response[0].in(),
        outList: chan2OutList)

    def rfl1 = new NodeRequestingSeqCastList(request: request[1].out(),
