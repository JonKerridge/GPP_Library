...
/**
  An AnyGroupOfPipelines comprises a network of {@code groups} parallel pipeline occurrences and each pipeline comprises
  {@code stages} Worker processes.
  Inputs from the preceding process are assumed to come from the any end of a channel.  Similarly, outputs go to an Any-ended channel.
  Internally, a collection of
  {@code groups} {@link groovyParallelPatterns.functionals.pipelines.OnePipelineOne OnePipelineOne} pipelines are created.
  ...
  @param stageModifier a list of {@code groups} lists, each containing {@code stages} elements that are the
  			stage modifier data for that combination of group and stage. Each entry is itself  a list of values.
  ...
  otherwise the process will not be logged.  Specific stages in the Pipeline can be logged by giving a string value, otherwise the value must
  be an empty string.  Thus some of the pipeline stages can be logged while others are not. Thus ["first", "", "third"] will result in the
  pipeline processes 0 and 2 being logged and labelled {@code first} and {@code third}.  Process with index 1 will not be logged.
