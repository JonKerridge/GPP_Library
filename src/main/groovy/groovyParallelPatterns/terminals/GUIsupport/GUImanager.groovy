package groovyParallelPatterns.terminals.GUIsupport
    ...
    void run(){
        ...
        while (!(inputObject instanceof UniversalTerminator)){
            ...
            if (retCode == DataClassInterface.completedOK )
                ...
                groovyParallelPatterns.DataClass.unexpectedReturnCode("GUImanager: error while displaying", retCode)
        }
        retCode = rc.&"${guiDetails.rFinaliseMethod}"( guiDetails.rFinaliseData )
        if (retCode != DataClassInterface.completedOK)
            groovyParallelPatterns.DataClass.unexpectedReturnCode("GUImanager: error while finalising", retCode)
