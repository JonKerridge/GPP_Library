 ...
 * {@link groovy_parallel_patterns.functionals.composites.AnyGroupOfPipelines composites} package.
 * These are either Groups of Pipelines or
 * Pipelines of Groups.  The data structure cDetails is a 2-dimensional array of
 * {@link groovy_parallel_patterns.LocalDetails LocalDetails};
 ...
 * A method is provided to insert the details into the object called {@code insertCompositeDetails}.<p>
 *
 ...
 * @return {@code GroupDetails} a list of {@code workers} {@link groovy_parallel_patterns.LocalDetails} objects,
 ...
 * @return {@code PipelineDetails} a list of {@code stages} {@link groovy_parallel_patterns.LocalDetails} objects,
 ...
 * */
	...
	PipelineDetails extractByPipe(int pipe){
		...
		PipelineDetails pDetails = new PipelineDetails(stages)
...
//        println "$cDetails"
	...
	/** Used to insert specific instances of a {@link groovy_parallel_patterns.LocalDetails LocalDetails}
	 * object into the property {@code cDetails}
	...
	String toString(){
		...
		for ( w in 0 ..< workers){
			for ( s in 0..< stages){
				str = str + "w: $w; s: $s -> ${cDetails[w][s].toString()}\n"
			}
		}
		return str
	}
